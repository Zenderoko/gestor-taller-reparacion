...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 flex flex-col bg-white border-r border-secondary-200 transition-all duration-300 lg:static lg:z-auto',
          collapsed ? 'w-16' : 'w-64',
      ...
      >
        <div className="flex items-center justify-between px-5 h-16 border-b border-secondary-100">
          ...
          <button
            onClick={onClose}
            className="p-1 text-secondary-400 hover:text-secondary-600 rounded-lg hover:bg-secondary-100 transition-colors lg:hidden"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
